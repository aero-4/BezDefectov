...
async def test_delete_card(clear_db):
    async with AsyncClient(base_url=base_url) as client:
        ...
        response3 = await client.delete(f"/api/cards/{card.id}")

        assert response3.json() is None

        response4 = await client.delete(f"/api/cards/{card.id}")
...
async def test_update_card(clear_db):
    async with AsyncClient(base_url=base_url) as client:
        ...
        response3 = await client.patch(f"/api/cards/{card.id}", json=card_update_dto.model_dump())
